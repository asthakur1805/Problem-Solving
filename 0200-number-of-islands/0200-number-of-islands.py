...
class Solution:
	...
	def numIslands(self,grid):
		...
		numRows, numColumns, visited, result, directions = len(grid), len(grid[0]), set({}), 0, [(-1,0),(1,0),(0,-1),(0,1)]

		for startRow in range(numRows):

			for startColumn in range(numColumns):

				if (startRow,startColumn) not in visited and grid[startRow][startColumn] == "1":
